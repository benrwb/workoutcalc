...
function getHeadline_internal(weight: number, reps: number[]): [number,string,number,number] {
    ...
    let average = arrayAverage(reps); // average including decimal
    let showTilde = average != maxReps;
    let roundedAverage = Math.round(average); // average rounded to nearest whole number
